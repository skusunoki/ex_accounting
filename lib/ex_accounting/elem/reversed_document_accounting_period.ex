defmodule ExAccounting.Elem.ReversedDocumentAccountingPeriod do
  ...
  @typedoc "_Reversed Document Accounting Period_"
  @type t :: %__MODULE__{reversed_document_accounting_period: AccountingPeriod.t()}
  defstruct reversed_document_accounting_period: nil

  @typedoc "_Accounting Period_"
  @type accounting_period :: AccountingPeriod.t()

  @doc """
  Define database field type of _Reversed Document Accounting Period_.
  """
  @spec type() :: :integer
  def type, do: :integer

  @doc """
  Casts the given _Accounting Period_ to the _Reversed Document Accounting Period_.
  ...
  def cast(%__MODULE__{} = reversed_document_accounting_period) do
    with %__MODULE__{reversed_document_accounting_period: reversed} <-
           reversed_document_accounting_period,
         %AccountingPeriod{accounting_period: _document_period} <- reversed do
      {:ok, reversed_document_accounting_period}
    else
      _ -> :error
    end
  end
  ...
  @doc """
  Dumps the _Reversed Document Accounting Period_ into the database form.

  ## Examples

      iex> dump(%ReversedDocumentAccountingPeriod{reversed_document_accounting_period: %AccountingPeriod{accounting_period: 12}})
      {:ok, 12}
  """
  @spec dump(t) :: {:ok, integer} | :error
  def dump(%__MODULE__{reversed_document_accounting_period: reversed_document_accounting_period}) do
    {:ok, reversed_document_accounting_period.accounting_period}
  end

  def dump(_), do: :error

  @doc """
  Loads the _Reversed Document Accounting Period_ from the database form data.
