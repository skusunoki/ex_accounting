defmodule ExAccounting.CurrentStatusTest do
  ...
  test "Read current document number for number range 01 is 1_000_000_001" do
    result = CurrentAccountingDocumentNumber.read("01")
    assert result.number_range_code == "01"
  ...
  test "Read current document number for number range 02 is 2_000_000_000" do
    result = CurrentAccountingDocumentNumber.read("02")
    assert result.number_range_code == "02"
  ...
  test "Read current document number for undefined number range 10 is nil" do
    result = CurrentAccountingDocumentNumber.read("10")
    assert result == nil
  end
